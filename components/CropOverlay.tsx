...
export const CropOverlay = ({ cropBoxStyle }: CropOverlayProps) => {
  ...
  const imageLayout = useConfirmStore.getState().photo
    ? { width: useConfirmStore.getState().photo.width, height: useConfirmStore.getState().photo.height }
    : { width: 1, height: 1 };

  const makePanResponder = (
    type: 'top' | 'bottom' | 'left' | 'right' | 'box'
  ) =>
    PanResponder.create({
      ...
      onPanResponderMove: (_, g) => {
        ...
        setCrop((prev) => {
          let { leftRatio, topRatio, widthRatio, heightRatio } = prev;
          ...
          if (type === 'box') {
            ...
            return height > 0.05 ? { ...prev, topRatio: Math.max(0, top), heightRatio: Math.min(1, height) } : prev;
          } else if (type === 'bottom') {
            const height = heightRatio + dy / h;
            ...
            return width > 0.05 ? { ...prev, leftRatio: Math.max(0, left), widthRatio: Math.min(1, width) } : prev;
          } else if (type === 'right') {
            const width = widthRatio + dx / w;
  ...
  return (
    ...
    >
      ...
      <View style={{ position: 'absolute', top: -10, left: -20, right: -20, height: 20, backgroundColor: '#4af', opacity: 0.6 }} {...panHandlers.top} />
      <View style={{ position: 'absolute', bottom: -10, left: -20, right: -20, height: 20, backgroundColor: '#4af', opacity: 0.6 }} {...panHandlers.bottom} />
      <View style={{ position: 'absolute', top: -20, bottom: -20, left: -10, width: 20, backgroundColor: '#4af', opacity: 0.6 }} {...panHandlers.left} />
      <View style={{ position: 'absolute', top: -20, bottom: -20, right: -10, width: 20, backgroundColor: '#4af', opacity: 0.6 }} {...panHandlers.right} />
    </View>
  );
};
